...
function absInjectGmTab(){
  ...
  const tabs=ABS_APP.querySelector('.gmt-tabs');
  if(!tabs||tabs.querySelector('[data-abs-gm-open]'))return;
  const b=document.createElement('button');
  b.type='button';b.dataset.absGmOpen='1';b.textContent='Yetenek Atölyesi';
  tabs.appendChild(b);
...
function absAbilityCard(a){
  const pills=[absEffectLabel(a.effect_type),'Hedef: '+absTargetLabel(a.target_type)];
  if(a.formula)pills.push(a.formula);
  if(a.requires_attack)pills.push('Saldırı '+(a.attack_bonus>=0?'+':'')+absNum(a.attack_bonus));
  return `<article class="abs-card"><div class="eyebrow abs-type-${absEsc(a.ability_type)}">${absTypeLabel(a.ability_type)}</div><h3>${absEsc(a.name)}</h3><div class="abs-pills">${pills.map(x=>`<span class="abs-pill">${absEsc(x)}</span>`).join('')}</div>${a.description?`<div class="mini muted">${absEsc(a.description)}</div>`:''}<div class="actions" style="margin-top:9px"><button type="button" data-abs-edit="${absEsc(a.id)}">Düzenle</button><button type="button" class="danger" data-abs-delete="${absEsc(a.id)}">Sil</button></div></article>`;
}

function absAssignmentsHtml(d){
  if(!d.assignments.length)return'<div class="muted">Henüz oyuncuya atanmış yetenek yok.</div>';
  return d.assignments.map(x=>{
    const c=d.chars.find(y=>y.id===x.character_id),a=d.abilities.find(y=>y.id===x.ability_id);
    const useText=x.uses_per_combat==null?'Sınırsız kullanım':x.uses_per_combat+' / savaş';
    return `<div class="abs-assignment"><div><b>${absEsc(c?.name||'Karakter')} • ${absEsc(a?.name||'Yetenek')}</b><div class="mini muted">${useText}</div></div><button type="button" class="danger small" data-abs-unassign="${absEsc(x.id)}">Kaldır</button></div>`;
  }).join('');
}
...
function absFormArgs(){
  return{
    p_name:ABS_APP.querySelector('#abs-name')?.value.trim()||'',
    p_ability_type:ABS_APP.querySelector('#abs-type')?.value||'skill',
    p_effect_type:ABS_APP.querySelector('#abs-effect')?.value||'damage',
    p_target_type:ABS_APP.querySelector('#abs-target')?.value||'enemy',
    p_formula:ABS_APP.querySelector('#abs-formula')?.value.trim()||'',
    p_requires_attack:ABS_APP.querySelector('#abs-requires-attack')?.value==='1',
    p_attack_bonus:absNum(ABS_APP.querySelector('#abs-attack-bonus')?.value),
    p_description:ABS_APP.querySelector('#abs-description')?.value.trim()||''
  };
}
...
async function absPlayerData(){
  const [ar,sr]=await Promise.all([ABS_S.rpc('catlak_player_abilities'),ABS_S.rpc('catlak_player_combat_snapshot')]);
  if(ar.error)throw ar.error;if(sr.error)throw sr.error;
  return{abilities:ar.data||[],snap:sr.data||{}};
}
...
function absPlayerCard(a,s){
  const targets=absTargetsFor(a,s);
  const uses=a.uses_per_combat==null?'∞':absNum(a.uses_remaining)+'/'+absNum(a.uses_per_combat);
  const out=a.uses_remaining!=null&&absNum(a.uses_remaining)<=0;
  const canTurn=!!s.is_my_turn,needsTarget=a.target_type!=='self';
  const canUse=canTurn&&!out&&(!needsTarget||targets.length>0);
  const pills=[];pills.push('Hedef: '+absTargetLabel(a.target_type));if(a.formula)pills.push(a.formula);pills.push('Kullanım: '+uses);if(a.requires_attack)pills.push('Saldırı '+(a.attack_bonus>=0?'+':'')+absNum(a.attack_bonus));
  let targetHtml='';
  if(needsTarget){const opts=targets.length?targets.map(x=>`<option value="${absEsc(x.id)}">${absEsc(x.name)} • HP ${absNum(x.hp_current)}/${absNum(x.hp_max)}</option>`).join(''):'<option value="">Uygun hedef yok</option>';targetHtml=`<label class="abs-target">Hedef<select data-abs-target-select="${absEsc(a.assignment_id)}">${opts}</select></label>`}
  const buttonText=out?'Kullanım Hakkı Bitti':canTurn?'Kullan':'Sıra Sende Değil';
  const desc=a.description?`<div class="mini muted">${absEsc(a.description)}</div>`:'';
  return `<article class="abs-player-card ${canUse?'ready':''}"><div class="eyebrow abs-type-${absEsc(a.ability_type)}">${absTypeLabel(a.ability_type)} • ${absEffectLabel(a.effect_type)}</div><h3>${absEsc(a.name)}</h3><div class="abs-pills">${pills.map(x=>`<span class="abs-pill">${absEsc(x)}</span>`).join('')}</div>${desc}${targetHtml}<button type="button" class="primary widebtn" data-abs-use="${absEsc(a.assignment_id)}" ${canUse?'':'disabled'}>${buttonText}</button></article>`;
}
function absResultHtml(r){
  if(!r)return'';
  let msg='Kullanıldı';
  if(r.hit===false)msg='ISKA';else if(r.effect_type==='damage')msg=absNum(r.amount)+' hasar';else if(r.effect_type==='heal')msg=absNum(r.amount)+' iyileştirme';
  const hp=r.target_hp_max!=null?' • HP '+absNum(r.target_hp)+'/'+absNum(r.target_hp_max):'';
  return `<div class="abs-result ${r.hit===false?'bad':'good'}" data-abs-result><div class="eyebrow">SON YETENEK</div><b>${absEsc(r.ability||'Yetenek')} → ${absEsc(r.target_name||'')}</b><div>${absEsc(msg+hp)}</div></div>`;
}
async function absRenderPlayer(force=false){
  if(absIsGM()||window.__catlakBattleRoomOpen!==true)return;
  const main=ABS_APP.querySelector('main');if(!main)return;
  try{
    const d=await absPlayerData();
    if(ABS_APP.querySelector('main')!==main||window.__catlakBattleRoomOpen!==true)return;
    const sig=JSON.stringify([d.snap?.is_my_turn,d.snap?.current_combatant_id,(d.snap?.order||[]).map(x=>[x.id,x.hp_current,x.hp_max,x.kind]),d.abilities.map(a=>[a.assignment_id,a.uses_remaining,a.name])]);
    if(!force&&sig===absPlayerSig&&main.querySelector('[data-abs-player]'))return;
    absPlayerSig=sig;main.querySelector('[data-abs-player]')?.remove();
    const sec=document.createElement('section');sec.className='card';sec.dataset.absPlayer='1';
    const title=d.abilities.length?d.abilities.length+' Aksiyon':'Henüz Yetenek Yok';
    const body=d.abilities.length?`<div class="abs-player-grid">${d.abilities.map(a=>absPlayerCard(a,d.snap)).join('')}</div>`:`<div class="muted">GM, Yetenek Atölyesi'nden karakterine yetenek verebilir.</div>`;
    sec.innerHTML=`<div class="eyebrow">YETENEKLER & BÜYÜLER</div><h2>${title}</h2><p class="muted">GM tarafından verilen büyü ve özel yetenekler burada kullanılır.</p>${body}${absResultHtml(absLastResult)}`;
    const weapon=[...main.querySelectorAll('section.card')].find(s=>absText(s.querySelector('.eyebrow'))==='TAKILI SİLAHLAR');
    weapon?weapon.insertAdjacentElement('afterend',sec):main.appendChild(sec);
  }catch(e){if(force)absToast('Yetenekler yüklenemedi: '+(e?.message||String(e)))}
}
async function absUse(btn){
  const id=btn.dataset.absUse;
  const sel=ABS_APP.querySelector(`[data-abs-target-select="${CSS.escape(id)}"]`);
  const target=sel?.value||null;
  const r=await ABS_S.rpc('catlak_player_use_ability',{p_assignment_id:id,p_target_id:target});if(r.error)throw r.error;
  absLastResult=r.data||null;absPlayerSig='';const d=r.data||{};
  let msg=d.ability||'Yetenek';if(d.hit===false)msg+=': ISKA';else if(d.effect_type==='damage')msg+=`: ${d.amount} hasar`;else if(d.effect_type==='heal')msg+=`: ${d.amount} iyileştirme`;else msg+=' kullanıldı';absToast(msg);
  if(window.__catlakRoomSystemTest?.renderBattle)await window.__catlakRoomSystemTest.renderBattle(true);
  await absRenderPlayer(true);
}
