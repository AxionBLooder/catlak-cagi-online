...
let managerOpen=false,playerOpen=false,busy=false,playerParty=false,owned=[],navQueued=false,renderToken=0,managerRefreshTimer=0;
...
function card(c){const d=c.data||{},p=d[PKEY]===true;return `<article class="prsv2-card" data-prsv2-char="${esc(c.id)}"><div class="prsv2-head"><div><div class="eyebrow" data-prsv2-owner>${c.owner_id?'OYUNCUYA BAĞLI':'HAZIR KARAKTER'}</div><h2 data-prsv2-name>${esc(c.name)}</h2><div class="muted" data-prsv2-meta>${esc(c.species_name||'')} • ${esc(c.class_name||'')} • Seviye ${Number(c.level||1)}</div></div><span class="prsv2-flag ${c.play_status==='active'?'on':''}" data-prsv2-status>${c.play_status==='active'?'CANLI':'HAZIR'}</span></div><div class="prsv2-flags"><span class="prsv2-flag ${p?'on':''}" data-prsv2-party-flag>${p?'✓ PARTİDE':'PARTİ DIŞI'}</span></div><div class="muted" data-prsv2-stats>HP ${Number(c.hp_current||0)}/${Number(c.hp_max||0)} • AC ${Number(c.base_ac||0)} • Hız ${Number(c.base_speed||0)}</div><div class="prsv2-actions"><button type="button" class="${p?'danger':'primary'}" data-prsv2-party="${esc(c.id)}" data-v="${p?'0':'1'}">${p?'Partiden Çıkar':'Partiye Al'}</button></div></article>`}
function managerListMarkup(chars){return `<div class="section-title"><div><div class="eyebrow">KATILIMCI HAVUZU</div><h2 data-prsv2-count>${chars.length} Karakter</h2></div></div>${chars.length?`<div class="prsv2-grid">${chars.map(card).join('')}</div>`:'<div class="empty">Hazır veya aktif karakter yok.</div>'}`}
function syncCardNode(node,c){if(!node)return;const p=c.data?.[PKEY]===true,set=(sel,v)=>{const el=node.querySelector(sel);if(el&&el.textContent!==String(v))el.textContent=String(v)};set('[data-prsv2-owner]',c.owner_id?'OYUNCUYA BAĞLI':'HAZIR KARAKTER');set('[data-prsv2-name]',c.name||'');set('[data-prsv2-meta]',`${c.species_name||''} • ${c.class_name||''} • Seviye ${Number(c.level||1)}`);set('[data-prsv2-stats]',`HP ${Number(c.hp_current||0)}/${Number(c.hp_max||0)} • AC ${Number(c.base_ac||0)} • Hız ${Number(c.base_speed||0)}`);const status=node.querySelector('[data-prsv2-status]');if(status){status.classList.toggle('on',c.play_status==='active');set('[data-prsv2-status]',c.play_status==='active'?'CANLI':'HAZIR')}applyPartyUi(c.id,p)}
function syncManagerSection(section,chars){const grid=section.querySelector('.prsv2-grid'),current=grid?[...grid.querySelectorAll('[data-prsv2-char]')]:[],currentIds=current.map(n=>String(n.dataset.prsv2Char||'')),nextIds=chars.map(x=>String(x.id));const same=grid&&currentIds.length===nextIds.length&&currentIds.every((id,i)=>id===nextIds[i]);if(!same){section.innerHTML=managerListMarkup(chars);return}const count=section.querySelector('[data-prsv2-count]');if(count)count.textContent=chars.length+' Karakter';for(let i=0;i<chars.length;i++)syncCardNode(current[i],chars[i])}
