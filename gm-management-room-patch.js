...
let roomOpen=false,busy=false,wrapTimer=0,refreshTimer=0,localHpUntil=0;
...
S.channel('gmr-characters').on('postgres_changes',{event:'*',schema:'public',table:'catlak_characters'},()=>{if(!roomOpen||Date.now()<localHpUntil)return;clearTimeout(refreshTimer);refreshTimer=setTimeout(()=>load(true),90)}).subscribe();
closeRoom();
ensureWrap();
