...
let dxBusy=false,dxGmBusy=false,dxKeepY=null,dxKeepUntil=0,dxPaintScheduled=false;
...
async function dxPaintGmDice(){
  if(!dxIsGM()||dxTab()!=='gm')return;
  ...
  let rolls=[];try{rolls=await dxLoadGmRolls()}catch(e){return}
...
async function dxGmRoll(kind){
  ...
  try{
    ...
    dxToast(`GM ${kind}: ${data?.total??'?'}`);await dxPaintGmDice();
  }catch(e){dxToast('GM zarı atılamadı: '+(e?.message||String(e)))}finally{dxGmBusy=false}
}
async function dxGmDelete(id){
  if(dxGmBusy||!dxIsGM())return;dxGmBusy=true;
  try{const {data,error}=await DX_S.rpc('catlak_delete_roll',{p_roll_id:Number(id)});if(error)throw error;if(data!==true)throw new Error('Zar silinemedi.');dxToast('GM zarı silindi.');await dxPaintGmDice()}catch(e){dxToast('GM zarı silinemedi: '+(e?.message||String(e)))}finally{dxGmBusy=false}
}
async function dxGmClear(){
  if(dxGmBusy||!dxIsGM())return;dxGmBusy=true;
  try{const {data,error}=await DX_S.rpc('catlak_gm_clear_private_rolls');if(error)throw error;dxToast(`${Number(data)||0} GM zarı temizlendi.`);await dxPaintGmDice()}catch(e){dxToast('GM zarları temizlenemedi: '+(e?.message||String(e)))}finally{dxGmBusy=false}
}
...
DX_S.channel('cc-dice-ux-live').on('postgres_changes',{event:'*',schema:'public',table:'catlak_rolls'},()=>setTimeout(dxSchedule,60)).subscribe();
dxSchedule();
