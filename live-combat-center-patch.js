...
async function addEnemies(){
 ...
 if(!name)return toast('Düşman adı gerekli.');
 actionBusy=true;
 let added=0;
 try{
  for(let i=0;i<count;i++){
   const display=count>1?`${name} ${i+1}`:name;
   const r=await S.rpc('catlak_gm_combat_add_enemy',{p_name:display,p_hp:hp,p_ac:ac,p_initiative:raw===''?null:Number(raw)});
   if(r.error)throw r.error;
   added++;
  }
  toast(added+' düşman savaşa eklendi'+(raw===''?' • her biri için otomatik inisiyatif atıldı.':'.'));
 }catch(e){
  toast('Düşman eklenemedi'+(added?' • '+added+' tanesi eklendi':'')+': '+(e?.message||String(e)));
 }finally{
  actionBusy=false;
  cachedBoardHtml='';
...
window.__catlakLiveCombatCenter={render:()=>render(true),restore:()=>restoreCachedBoard(APP.querySelector('main')),preload:preloadBoard,open:openLive,removeLegacy:removeOldCombatEntry,start:startCombat,addEnemies,actionBusy:()=>actionBusy};
})();