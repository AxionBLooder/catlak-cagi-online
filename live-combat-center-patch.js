...
function combatantHtml(x,current,all){
 ...
 return `<article class="lcc-combatant ${isCurrent?'current':''} ${targeted?'targeted':''}"><div class="lcc-combatant-top"><div class="lcc-init">${num(x.initiative)}</div><div><b>${esc(x.name||'Savaşçı')}</b><div class="lcc-mini">${kind} • AC ${x.ac??'?'}${isCurrent?' • SIRA BUNDA':''}</div><div class="lcc-hp">HP ${x.hp_current??'?'} / ${x.hp_max??'?'}</div></div><button type="button" class="danger small" data-lcc-remove="${esc(id)}">Çıkar</button></div><div class="lcc-actions"><button type="button" data-lcc-hp="${esc(id)}" data-d="-5">HP −5</button><button type="button" data-lcc-hp="${esc(id)}" data-d="-1">−1</button><button type="button" data-lcc-hp="${esc(id)}" data-d="1">+1</button><button type="button" data-lcc-hp="${esc(id)}" data-d="5">HP +5</button><button type="button" data-lcc-init="${esc(id)}">İnisiyatif At</button>${targetAction}</div>${targeted?'<div class="lcc-target-note">🎯 YARATIK HEDEFİ • Sonraki yaratık saldırısı bu oyuncuya gider.</div>':''}${attackBox}</article>`;
}
