...
if(!document.querySelector('#br3-style')){
  ...
  @media(min-width:1050px){
    #app main.br3-live-layout .ccr-battle-grid>div:first-child{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:12px!important;align-items:start!important}
    #app main.br3-live-layout [data-br3-turn],#app main.br3-live-layout .br3-order-section,#app main.br3-live-layout [data-br3-creatures],#app main.br3-live-layout [data-br3-hp]{grid-column:1/-1!important}
    #app main.br3-live-layout .br3-weapons-section{grid-column:1!important;margin:0!important}
    #app main.br3-live-layout [data-br3-abilities]{grid-column:2!important;margin:0!important}
  }
...
async function br3Load(){
  const [sr,ar,lr]=await Promise.allSettled([
  ...
  const logRes=lr.status==='fulfilled'?lr.value:null;
  const warnings=[];
  if(!abilityRes||abilityRes.error)warnings.push('Yetenek listesi geçici olarak alınamadı.');
  if(!logRes||logRes.error)warnings.push('Savaş günlüğü geçici olarak alınamadı.');
  return{
    snap:snapRes.data||{},
    abilities:abilityRes&&!abilityRes.error?(abilityRes.data||[]):[],
    logs:logRes&&!logRes.error?(logRes.data||[]):[],
    warnings
  };
}
...
function br3AbilityCard(a,s){
  ...
  const opts=targets.map(x=>`<option value="${br3Esc(x.id)}">${br3Esc(x.name)}</option>`).join('');
...
function br3AbilitiesSection(d){const warning=d.warnings?.some(x=>x.includes('Yetenek'))?'<div class="br3-warning">Yetenek listesi şu anda alınamadı; yaratık hedefleme çalışmaya devam eder.</div>':'';return `<section class="card" data-br3-abilities><div class="eyebrow">YETENEKLER & BÜYÜLER</div><h2>${d.abilities.length?d.abilities.length+' Aksiyon':'Henüz Yetenek Yok'}</h2><p class="muted">GM Merkezi → Yetenek bölümünden karakterine verilen büyü ve özel yetenekler burada görünür ve sıra sendeyken kullanılır.</p>${warning}${d.abilities.length?`<div class="br3-grid">${d.abilities.map(a=>br3AbilityCard(a,d.snap)).join('')}</div>`:'<div class="muted">Bu karaktere henüz bir yetenek atanmadı.</div>'}${br3ResultHtml(br3LastResult)}</section>`}
function br3ResultHtml(r){if(!r)return'';let text='Kullanıldı',bad=r.hit===false;if(bad)text='ISKA';else if(r.effect_type==='damage')text=br3Num(r.amount)+' hasar';else if(r.effect_type==='heal')text=br3Num(r.amount)+' iyileştirme';return `<div class="br3-result ${bad?'bad':'good'}"><div class="eyebrow">SON AKSİYON</div><b>${br3Esc(r.ability||'Yetenek')} → ${br3Esc(r.target_name||'')}</b><div>${br3Esc(text)}</div></div>`}
...
function br3LogSection(d){const warning=d.warnings?.some(x=>x.includes('günlüğ'))?'<div class="br3-warning">Savaş günlüğü geçici olarak alınamadı.</div>':'';const rows=d.logs||[];return `<section class="card" data-br3-log><div class="eyebrow">SAVAŞ GÜNLÜĞÜ</div><h2>Canlı Akış</h2>${warning}${rows.length?`<div class="br3-log">${rows.map(x=>`<div class="br3-log-row"><time>${br3Time(x.created_at)}</time><div><b>${x.kind==='turn'?'TUR':'SAVAŞ'}</b><div class="mini">${br3Esc(x.message)}</div></div></div>`).join('')}</div>`:'<div class="muted">Canlı akış temiz veya henüz savaş kaydı yok.</div>'}</section>`}
...
function br3Insert(d){
  ...
  [...main.querySelectorAll('section.card')].forEach(sec=>{if(br3Txt(sec.querySelector('.eyebrow'))==='HIZLI D20')sec.remove()});
  ...
  if(abilities)abilities=br3ReplaceIfChanged(abilities,nextAbilities);else{abilities=nextAbilities;const weapons=[...main.querySelectorAll('section.card')].find(s=>br3Txt(s.querySelector('.eyebrow'))==='TAKILI SİLAHLAR');if(weapons)weapons.classList.add('br3-weapons-section');weapons?weapons.insertAdjacentElement('afterend',abilities):(left?left.appendChild(abilities):main.appendChild(abilities))}
  const weapons=[...main.querySelectorAll('section.card')].find(s=>br3Txt(s.querySelector('.eyebrow'))==='TAKILI SİLAHLAR');if(weapons)weapons.classList.add('br3-weapons-section');
