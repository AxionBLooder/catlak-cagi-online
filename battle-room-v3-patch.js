...
function br3ResultHtml(r){
  if(!r)return'';
  const bad=r.hit===false;
  let text='Kullanıldı';
  if(bad){
    const check=r.attack_total!=null&&r.target_ac!=null?` • Saldırı ${br3Num(r.attack_total)} / AC ${br3Num(r.target_ac)}`:'';
    text='BAŞARISIZ'+check+' • Yetenek kullanıldı, hasar uygulanmadı';
  }else if(r.effect_type==='damage')text=br3Num(r.amount)+' hasar';
...
async function br3Use(btn){if(br3ActionBusy)return;const id=btn.dataset.br3Use,kind=btn.dataset.br3UseKind||'self',sel=BR3_APP.querySelector(`[data-br3-ability-target="${CSS.escape(id)}"]`),target=kind==='enemy'?(br3TargetId||null):(sel?.value||null),before=br3LastSnap;br3ActionBusy=true;try{const r=await BR3_S.rpc('catlak_player_use_ability',{p_assignment_id:id,p_target_id:target});if(r.error)throw r.error;br3LastResult=r.data||null;const d=r.data||{};window.__catlakRealtimeSync?.emit?.('combat',{action:'ability-use'});window.__catlakRealtimeSync?.emit?.('ability',{action:'used'});let msg=d.ability||'Yetenek';if(d.hit===false){const check=d.attack_total!=null&&d.target_ac!=null?` (${br3Num(d.attack_total)} / AC ${br3Num(d.target_ac)})`:'';msg+=`: BAŞARISIZ${check} • kullanım harcandı, hasar yok`}else if(d.effect_type==='damage')msg+=`: ${d.amount} hasar`;else if(d.effect_type==='heal')msg+=`: ${d.amount} iyileştirme`;else msg+=' kullanıldı';br3Toast(msg);if(!(d.effect_type==='damage'&&await br3MaybeAutoEndCombat(before)))await br3Render(false)}finally{br3ActionBusy=false}}
async function br3HpChange(btn){if(br3ActionBusy)return;br3ActionBusy=true;br3HoldInteraction(1500);try{const input=BR3_APP.querySelector('[data-br3-hp-amount]'),amount=Math.max(1,Math.abs(br3Num(input?.value,1))),d=br3LastData||await br3Load(),self=(d.snap?.order||[]).find(x=>x.is_self);if(!self||!d.snap?.character_id)throw new Error('Savaş karakteri bulunamadı.');const delta=btn.dataset.br3HpChange==='heal'?amount:-amount,next=Math.max(0,Math.min(br3Num(self.hp_max),br3Num(self.hp_current)+delta));const r=await BR3_S.rpc('catlak_update_my_hp',{p_character_id:d.snap.character_id,p_hp:next});if(r.error)throw r.error;self.hp_current=next;if(br3LastSnap){const mine=(br3LastSnap.order||[]).find(x=>x.is_self);if(mine)mine.hp_current=next}br3SyncHeroVitals(d.snap);br3Sig='';br3CacheCurrent();window.__catlakRealtimeSync?.emit?.('combat',{action:'player-hp'});br3Toast('HP '+next+'/'+br3Num(self.hp_max));setTimeout(()=>{if(!br3InteractionLocked())br3Render(false)},170)}finally{br3ActionBusy=false}}
