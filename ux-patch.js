...
let cuxChars=[],cuxSelected='',cuxBusy=false,cuxSeq=0,cuxDirty=false;
...
function cuxRouteLoading(label='Sayfa'){
  ...
  main.innerHTML=`<section class="card cux-route-loading"><div><div><span class="cux-pulse"></span><b>${cuxH(label)} açılıyor</b></div><p class="muted">Yeni içerik hazırlanıyor.</p></div></section>`;
...
function cuxCurrent(){return cuxChars.find(c=>String(c.id)===String(cuxSelected))||cuxChars[0]||null}
function cuxPaint({rebuildList=true,refreshEditor=!cuxDirty}={}){
  ...
  if(list){
    ...
    if(rebuildList)list.innerHTML=cuxChars.length?cuxChars.map(cuxCharButton).join(''):'<div class="cc-empty">Karakter yok.</div>';
    list.querySelectorAll('[data-cux-character]').forEach(b=>b.classList.toggle('on',String(b.dataset.cuxCharacter)===String(cuxSelected)));
    requestAnimationFrame(()=>{if(list.isConnected)list.scrollTop=y});
  }
  if(refreshEditor){
    const editor=workshop.querySelector('.cux-editor');
    if(editor)editor.outerHTML=cuxEditor(selected);else workshop.insertAdjacentHTML('beforeend',cuxEditor(selected));
    cuxDirty=false;
  }
  main.dataset.cuxPage='stats';
}
function cuxSelect(id){
  if(!cuxChars.some(c=>String(c.id)===String(id)))return;
  if(String(cuxSelected)===String(id)){cuxPaint({rebuildList:false,refreshEditor:false});return}
  cuxSelected=id;cuxDirty=false;cuxPaint({rebuildList:false,refreshEditor:true});
}

async function cuxRenderStats(force=false){
  if(!cuxIsGM()||!cuxStatsActive()||cuxBusy)return;
  const main=CUX_APP.querySelector('main');if(!main)return;if(!force&&main.dataset.cuxPage==='stats'&&main.querySelector('.cux-workshop'))return;
  ...
  try{
    ...
    cuxChars=data||[];
    if(!cuxSelected||!cuxChars.some(c=>String(c.id)===String(cuxSelected)))cuxSelected=cuxChars[0]?.id||'';
    cuxPaint({rebuildList:true,refreshEditor:!cuxDirty});
...
async function cuxSave(id){
  ...
  try{
    ...
    cuxDirty=false;cuxToast('Karakter değerleri kaydedildi.');await cuxRenderStats(true);
