...
function erNavLabel(){if(!erIsGM())return;const b=ER_APP.querySelector('.nav button[data-tab="rolls"]');if(b&&b.textContent!=='Zarlar')b.textContent='Zarlar'}
...
async function erRender(force=false){
  ...
  try{
    ...
    erStamp=stamp;
    main.innerHTML=`<section class="card"><div class="eyebrow">GM • ZARLAR</div><h1>Zarlar & Olaylar</h1><p class="muted">Sayıya basınca seçtiğin zar doğrudan atılır. Sağ taraftaki olay zarları yalnız GM tarafından görülür.</p></section><div class="er-wrap"><div class="er-stack"><section class="card"><div class="er-head"><div><div class="eyebrow">GM ÖZEL ZARLARI</div><h2>20 & 100</h2></div><button type="button" class="danger small" data-er-clear="private">GM Zarlarını Temizle</button></div><div class="er-quick"><div class="er-sub"><button type="button" class="primary er-roll-btn" data-er-private="d20">🎲 20</button><div class="er-chips">${erPrivateChips(priv,'gm_d20')}</div></div><div class="er-sub"><button type="button" class="primary er-roll-btn" data-er-private="d100">🎲 100</button><div class="er-chips">${erPrivateChips(priv,'gm_d100')}</div></div></div><div class="er-private">Bu sonuçlar oyunculara görünmez.</div></section><section class="card"><div class="er-head"><div><div class="eyebrow">OYUNCU ZARLARI</div><h2>Son Atışlar</h2></div><button type="button" class="danger small" data-er-clear="player">Oyuncu Zarlarını Temizle</button></div>${player.length?player.map(r=>erPlayerRow(r,chars)).join(''):'<div class="er-empty">Henüz oyuncu zarı yok.</div>'}</section></div><section class="card"><div class="er-head"><div><div class="eyebrow">GM • OLAY ZARLARI</div><h2>Rastgele Olaylar</h2></div><button type="button" class="danger small" data-er-clear="events">Olay Geçmişini Temizle</button></div><div class="er-events">${erEventCard('d6','Yolculuk / Karşılaşma','1 Tehlike · 2 Garip NPC · 3 Canavar · 4 Hazine · 5 Eski Dünya kalıntısı · 6 Tamamen saçma bir olay')}${erEventCard('d20','İyi / Kötü Sonuç','1–10 İyi şeyler · 11–20 Kötü şeyler')}${erEventCard('d100','Belirsiz Olay / Eşya','1–49 İyi şeyler olur · 50–100 Belirsiz öğeler ve eşyalar')}${erEventCard('d200','Nadir / Özel Ödül','1–100 Konuşan silahlar · 101–200 Mitik eşya')}</div><hr><div class="eyebrow">OLAY GEÇMİŞİ</div><div class="er-history">${events.length?events.map(erEventResult).join(''):'<div class="er-empty">Henüz olay zarı atılmadı.</div>'}</div><div class="er-private">Olay sonuçları yalnız GM hesabında görünür ve oyuncu zar geçmişine düşmez.</div></section></div>`;
    main.dataset.erRollCenter='1';
  }catch(e){erToast('Zarlar yüklenemedi: '+(e?.message||String(e)))}finally{erRendering=false}
}

async function erPrivateRoll(kind){
  if(erBusy||!erIsGM())return;erBusy=true;
  try{const fn=kind==='d100'?'catlak_gm_roll_d100':'catlak_gm_roll_d20';const {data,error}=await ER_S.rpc(fn);if(error)throw error;erToast(`GM ${erDieNumber(kind)}: ${data?.total??'?'}`);await erRender(true)}catch(e){erToast('GM zarı atılamadı: '+(e?.message||String(e)))}finally{erBusy=false}
}
async function erEventRoll(kind){
  if(erBusy||!erIsGM())return;erBusy=true;
  try{const {data,error}=await ER_S.rpc('catlak_gm_roll_event',{p_table:kind});if(error)throw error;erToast(`${erDieNumber(kind)}: ${data?.total??'?'} • ${data?.dm_ruling||erOutcomeFor(kind,data?.total)}`);await erRender(true)}catch(e){erToast('Olay zarı atılamadı: '+(e?.message||String(e)))}finally{erBusy=false}
}
