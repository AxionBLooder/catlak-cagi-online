...
if(!GMS_S||!GMS_APP)throw new Error('Çatlak Çağı GM silah dağıtım katmanı başlatılamadı.');
...
async function gmsItem(id){
  ...
  const r=await GMS_S.from('catlak_items').select('id,name,item_type').eq('id',id).maybeSingle();
...
function gmsSlotLabel(slot){
  return slot==='main_weapon'?'1. Silah':slot==='off_weapon'?'2. Silah':'Sadece Envantere';
}
...
function gmsEnsureSlotControl(){
  ...
  if(!select){
    ...
    label.innerHTML='Silah Yuvası<select id="gms-give-slot"><option value="">Sadece Envantere</option><option value="main_weapon">1. Silah</option><option value="off_weapon">2. Silah</option></select><small class="gms-slot-help" data-gms-slot-help>Silah seçersen hangi yuvaya takılacağını burada belirleyebilirsin.</small>';
...
async function gmsSyncSlotControl(){
  ...
  try{
    ...
    const weapon=item?.item_type==='weapon';
    select.disabled=!weapon;
    if(!weapon)select.value='';
    if(help)help.textContent=weapon?'Sadece envantere bırakabilir veya doğrudan 1./2. silaha takabilirsin.':'Bu kayıt silah değil; yalnız envantere verilebilir.';
...
async function gmsPaintInventorySlots(){
  ...
  for(const b of buttons){
    ...
    if(inv.equipped_slot!=='main_weapon'&&inv.equipped_slot!=='off_weapon')continue;
...
async function gmsGive(){
  ...
  try{
    ...
    if(slot&&item.item_type!=='weapon')throw new Error('1. ve 2. silah yuvasına yalnız silah verilebilir.');
    ...
    if(slot){
      ...
      if(!inv.data?.id)throw new Error('Silah envantere verildi ama yuva kaydı bulunamadı.');
      ...
      gmsToast(`${item.name} oyuncuya verildi ve ${gmsSlotLabel(slot)} olarak takıldı.`);
    }else{
...
window.__catlakGmItemSlotTest={install:gmsInstall,sync:gmsSyncSlotControl,give:gmsGive};