(() => {
  ...
  var __catlakBaseReady = (async () => {
    ...
    async function catload() {
      try {
        let b = await (await fetch("https://cdn.jsdelivr.net/gh/AxionBLooder/catlak-cagi-online@f3aaa7bf74e25020a4222b5e91e5f9e8d37fd6b7/catalog.b64")).text(), u = Uint8Array.from(atob(b.trim()), (c) => c.charCodeAt(0)), txt = await new Response(new Blob([u]).stream().pipeThrough(new DecompressionStream("gzip"))).text();
