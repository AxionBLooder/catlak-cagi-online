(() => {
  ...
  var __catlakQualityReady = (async () => {
    ...
    const ccqH = (x) => String(x != null ? x : "").replace(/[&<>"']/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[c]);
    const ccqToast = (x) => {
      const t = document.querySelector("#toast");
      if (!t) return;
      t.textContent = String(x);
      t.classList.remove("hidden");
      clearTimeout(ccqToast.t);
      ccqToast.t = setTimeout(() => t.classList.add("hidden"), 4200);
    };
    ...
    if (!document.querySelector("#ccq-style")) {
      const s = document.createElement("style");
      s.id = "ccq-style";
      s.textContent = ccqCss;
      document.head.appendChild(s);
    }
    function ccqMapActive() {
      return !!CCQ_APP.querySelector("[data-cc-map-tab].on");
    }
    async function ccqRenderOthers() {
      ...
      try {
        ...
        section.dataset.ccqOtherArchive = "1";
        section.innerHTML = '<div class="eyebrow">EVREN GÖRSELLERİ</div><h2>Bölgeler & Sahneler</h2><p class="muted">Harita olmayan Evren görselleri burada ayrı tutulur. Yine de buradan canlı partiye yansıtabilirsin.</p>'.concat(rows.length ? '<div class="ccq-world-grid">'.concat(rows.map((x) => '<article class="card ccq-card"><span class="tag">EVREN</span><h3>'.concat(ccqH(x.title || "Adsız Görsel"), "</h3>").concat(x.image_url ? '<div class="ccq-frame"><img src="'.concat(ccqH(x.image_url), '" alt="').concat(ccqH(x.title || "Evren görseli"), '" loading="lazy"></div>') : "", "<p>").concat(ccqH(x.description || ""), '</p><div class="ccq-actions"><button type="button" class="primary" data-ccq-party-other="').concat(x.id, '">Partiye Yansıt</button></div><div class="ccq-quality-note">Eski düşük çözünürlüklü arşiv görselleri doğal boyutuna yakın gösterilir; yapay olarak karta büyütülmez.</div></article>')).join(""), "</div>") : '<div class="cc-world-empty">Evren görseli yok.</div>');
        current.appendChild(section);
      } catch (e) {
        console.error("CCQ_RENDER", e);
      } finally {
        ccqBusy = false;
      }
    }
    async function ccqPushOther(id) {
      ...
      if (error) throw error;
      const u = await CCQ_S.from("catlak_party_visual").update({ kind: "other", ref_id: id, title: row.title || "", image_url: row.image_url || "", note: row.description || "", updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("singleton", true).select("singleton");
      if (u.error) throw u.error;
      if (!(u.data || []).length) throw new Error("Parti görseli güncellenemedi.");
      ccqToast((row.title || "Evren görseli") + " partiye yansıtıldı.");
    }
    function ccqSchedule(delay = 80) {
      clearTimeout(ccqTimer);
      ccqTimer = setTimeout(ccqRenderOthers, delay);
    }
    document.addEventListener("click", (e) => {
      if (e.target.closest("[data-cc-map-tab]")) ccqSchedule(180);
      const p = e.target.closest("[data-ccq-party-other]");
      if (p) {
        e.preventDefault();
        e.stopImmediatePropagation();
        ccqPushOther(p.dataset.ccqPartyOther).catch((err) => ccqToast("Partiye yansıtılamadı: " + ((err == null ? void 0 : err.message) || String(err))));
        return;
      }
    }, true);
    CCQ_S.channel("ccq-gallery-live").on("postgres_changes", { event: "*", schema: "public", table: "catlak_world_media" }, () => {
      var _a;
      (_a = CCQ_APP.querySelector("[data-ccq-other-archive]")) == null ? void 0 : _a.remove();
      ccqSchedule(120);
    }).subscribe();
