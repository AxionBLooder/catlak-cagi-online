(() => {
  ...
  var __catlakBaseReady = (async () => {
    ...
    async function catload() {
      try {
        let b = await (await fetch("https://cdn.jsdelivr.net/gh/AxionBLooder/catlak-cagi-online@01cd95c2c8cd58e5b282c84d091c3ebd0c55dd2a/catalog.b64")).text(), u = Uint8Array.from(atob(b.trim()), (c) => c.charCodeAt(0)), txt = await new Response(new Blob([u]).stream().pipeThrough(new DecompressionStream("gzip"))).text();
