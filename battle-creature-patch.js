...
let bccBusy=false,bccTimer=null,bccPlayerGen=0,bccLastSig='';
...
if(!document.querySelector('#bcc-style')){
  ...
  .bcc-creature{border:1px solid var(--line);border-radius:14px;padding:13px;background:#0a1622;position:relative;overflow:hidden}
...
function bccCreatureCard(x){
  const hp=bccNum(x.hp_current),max=bccNum(x.hp_max),dead=max>0&&hp<=0;
  const attack=[x.attack_name,x.attack_formula].filter(Boolean).map(bccEsc).join(' • ');
  return `<article class="bcc-creature ${x.is_current?'current':''} ${dead?'dead':''}">
...
async function bccPaintPlayer(force=false){
  ...
  try{
    ...
    const sig=JSON.stringify(enemies.map(x=>[x.id,x.hp_current,x.hp_max,x.ac,x.initiative,x.name,x.creature_type,x.attack_name,x.attack_formula,x.damage_formula,x.public_note,x.is_current]));
    if(!force&&sig===bccLastSig&&main.querySelector('[data-bcc-creatures]'))return;bccLastSig=sig;
    main.querySelector('[data-bcc-creatures]')?.remove();
    if(!s.active||!enemies.length)return;
    const sec=document.createElement('section');sec.className='card';sec.dataset.bccCreatures='1';
    sec.innerHTML=`<div class="eyebrow">KARŞILAŞMADAKİ YARATIKLAR</div><h2>${enemies.length} Yaratık</h2><div class="bcc-creature-grid">${enemies.map(bccCreatureCard).join('')}</div>`;
  ...
  }catch(e){if(force)bccToast('Yaratıklar yüklenemedi: '+(e?.message||String(e)))}
}
...
document.addEventListener('click',e=>{
  const b=e.target.closest?.('[data-bcc-add-creature]');if(!b)return;
  e.preventDefault();e.stopImmediatePropagation();bccAddCreature(b);
},true);
...
window.__catlakBattleCreatureTest={install:bccInstall,paint:bccPaintPlayer};
